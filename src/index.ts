/* eslint-disable */
export * from './components/parallax';
export * from './components/parallax-spring';
export * from './contexts/parallax.provider';

export * as utils from './utils';
export * from './types/transform';

export { default as Axis } from './enums/axis';
export { default } from './components/parallax';
export { default as ParallaxSpring } from './components/parallax-spring';
export { default as ParallaxProvider } from './contexts/parallax.provider';
